...
static void lpreson_tilde_free(t_lpreson_tilde *x)
{
  if (x->x_ctl.x_parcors)
    freebytes(x->x_ctl.x_parcors, x->x_order * sizeof(t_atom));
}
...
void *lpreson_tilde_new(t_floatarg f)
{
  ...
  x->x_ctl.x_parcors = (t_atom *)getbytes(x->x_order * sizeof(t_atom));
  if (x->x_ctl.x_parcors)
  {
    int i;
    for (i = 0; i < x->x_order; i++)
      SETFLOAT(&x->x_ctl.x_parcors[i], 0);
  }
...
{
  lpreson_tilde_class = class_new(gensym("lpreson~"), (t_newmethod)lpreson_tilde_new, (t_method)lpreson_tilde_free, sizeof(t_lpreson_tilde), CLASS_DEFAULT, A_DEFFLOAT, 0);
